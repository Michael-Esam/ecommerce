...
import { insertBrandSchema } from '@/lib/db/schema/filters/brands';
import { insertCategorySchema } from '@/lib/db/schema/categories';
import { insertCollectionSchema } from '@/lib/db/schema/collections';
import { insertProductSchema, insertProductImageSchema, type InsertProduct, type InsertProductImage } from '@/lib/db/schema/products';
import { insertVariantSchema, type InsertVariant } from '@/lib/db/schema/variants';
...
async function seed() {
    try {
        ...
        for (let i = 0; i < productNames.length; i++) {
            ...
            for (const color of colorChoices) {
                for (const size of sizeChoices) {
                    ...
                    const variant = insertVariantSchema.parse({
                        ...
                        weight: (Math.random() * 1 + 0.5).toFixed(2),
            ...
            for (let j = 0; j < 4; j++) {
                const pickName = sourceImages[(i + j) % sourceImages.length];
                const src = join(sourceDir, pickName);
                const destName = `${insertedProduct.id}-${j}-${basename(pickName)}`;
